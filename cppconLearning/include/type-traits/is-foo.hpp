...
#include "is-class.hpp"
#include "is-noncv-foo.hpp"
...
namespace ft
{
    ...
    template <typename T> struct is_floating_point          : is_noncv_floating_point<remove_cv_t<T>> {};

    template <typename T> struct is_integral                : is_noncv_integral<remove_cv_t<T>> {};

    template <typename T> struct is_member_pointer          : is_noncv_member_pointer<remove_cv_t<T>> {};

    template <typename T> struct is_null_pointer            : is_noncv_null_pointer<remove_cv_t<T>> {};

    template <typename T> struct is_pointer                 : is_noncv_pointer<remove_cv_t<T>> {};
    ...
    // Some more traits based on the traits above 
    // Every type is either compound or fundamental
